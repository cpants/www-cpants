...
const my %IndicatorLevel => map { $_->{name} => ($_->{is_experimental} ? $Level{experimental} : $_->{is_extra} ? $Level{extra} : $Level{core}) } @Indicators;

my @SortedIndicators = sort { $IndicatorLevel{ $a->{name} } <=> $IndicatorLevel{ $b->{name} } or $a->{name} cmp $b->{name} } @Indicators;
