...
sub mark_implicit_latest {
  my $self = shift;
  $self->do("update kwalitee set is_latest = 1 where distv in (select distv from kwalitee group by dist having ifnull(min(is_latest), 0) < 1 and released = max(released))");
}
